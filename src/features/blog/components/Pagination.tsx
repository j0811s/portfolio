...
export default function Pagination({ pager, type }: PaginationParam) {
    ...
    return (
      <>
        <li className={`${styles.listItem}`}>
          <Link className={styles.pageLink} href={`${urlPath}${1}`}>1</Link>
        </li>
  ...
  const rangePageNumbers = pageCount.map(page => {
    if (currentPage - 3 <= page && page <= currentPage + 1) {
      return (
        <li key={page} className={`${styles.listItem} ${currentPageCheck(page) ? currentPage : ''}`}>
          {
            currentPageCheck(page)
              ? <div className={`${styles.currentText} ${styles.pageLink}`}>{page + 1}</div>
              : <Link className={styles.pageLink} href={`${urlPath}${page + 1}`}>{page + 1}</Link>
          }
        </li>
      )
    }
    ...
    if (currentPage + 2 < pageCount.length) {
      return (
        <>
          ...
          <li className={`${styles.listItem}`}>
            <Link className={styles.pageLink} href={`${urlPath}${pageCount.length}`}>{pageCount.length}</Link>
          </li>
        </>
      )
    }
