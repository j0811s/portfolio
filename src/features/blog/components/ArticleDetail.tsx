...
import parse, { type HTMLReactParserOptions, Element, type Text } from 'html-react-parser';
import hljs, { type AutoHighlightResult } from 'highlight.js';
    ...
    switch (domNode.name) {
      case 'pre': {
        const fileName: string =
          (domNode.parent as Element)?.attribs['data-filename'] !== ''
            ? (domNode.parent as Element)?.attribs['data-filename']
            : '';
...
async function ArticleDetail({ post }: { post: Blog }) {
  ...
  const eyecatchPath =
    typeof post.eyecatch?.url === 'undefined'
      ? undefined
      : `${post.eyecatch.url}?auto=format&w=880&ar=16:9&fit=crop&q=50`;
  ...
  return (
    <article id={post.id} className={styles.postWrapper}>
      ...
      <div className={styles.backToLink}>
        <CtaLinkButton href="/blog/" prevIcon={true}>
          一覧へ戻る
        </CtaLinkButton>
      </div>
    </article>
